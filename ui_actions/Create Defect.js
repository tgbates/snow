...
function createDefect() {
	...
	// insert new record on defect table
	var taskSys = task.insert();

	// copy attachments
	GlideSysAttachment.copy('rm_enhancement', current.sys_id, 'rm_defect', taskSys);


	// close current record
	current.comments = 'Cancelled ' + current.number + ' and created defect ' + task.number + ' on ' + gs.nowDateTime();
	current.state = 7;
	current.update();
